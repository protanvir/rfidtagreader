...
from tkinter import Tk, Button, Label, Listbox, Scrollbar, END, RIGHT, LEFT, Y, BOTH, Frame, simpledialog
import queue
...
def read_tags():
    ...
    try:
        ...
        ip_address = simpledialog.askstring("Input", "Enter RFID Reader IP Address:",
                                          initialvalue="192.168.2.2")
        if ip_address is None:  # User clicked Cancel
            reading = False
            status_label.config(text="Status: Cancelled")
            return
            
        reader = llrp.LLRPReaderClient(host=ip_address, port=5084, config=config)
        reader.add_tag_report_callback(tag_report_callback)
        reader.connect()
        status_label.config(text=f"Status: Connected to {ip_address}")
