    ...
    for tag in tag_reports:
        ...
        if epc != 'UNKNOWN':  # Only process valid EPCs
            ...
            if epc not in seen_tags:
                seen_tags.add(epc)
                print(f"[DEBUG] Adding to queue: EPC={epc}, Timestamp={timestamp}")  # Debugging: Log additions to the queue
...
def process_gui_updates():
    while not gui_update_queue.empty():
        ...
        print(f"[DEBUG] Processing GUI Update: EPC={epc}, Timestamp={timestamp}")  # Debugging: Log updates being processed
        try:
            ...
            print(f"[DEBUG] GUI Updated: EPC={epc}, Total Tags={len(seen_tags)}")  # Debugging: Confirm GUI update
        except Exception as e:
            print(f"[ERROR] GUI Update Failed: {e}")  # Debugging: Log GUI update errors
    print("[DEBUG] GUI update queue processed.")  # Debugging: Log when the queue is processed
